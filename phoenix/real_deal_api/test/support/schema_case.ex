defmodule RealDealApi.Support.SchemaCase do
	...
	def invalid_params(fields_with_types) do
		invalid_value_by_type = %{
			...
			naive_datetime: fn -> Faker.UUID.v4() end
		}
		for {field, type} <- fields_with_types, into: %{} do
			case field do
				:email -> {Atom.to_string(field), Faker.NaiveDateTime.backward(Enum.random(0..100))}
				_ -> {Atom.to_string(field), invalid_value_by_type[type].()}
			end
